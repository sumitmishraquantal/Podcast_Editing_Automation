# """
# run_client.py - fully automated single-client reel run.

#     fetch -> pipeline (6 stages) -> qc -> [self-correct] -> caption -> archive (only if shippable)

#   - STRICT QC gate: if objective checks fail after retries, DO NOT archive.
#   - Self-correction: re-run the relevant stage, max 2 attempts per issue.
#   - On shippable: generate Groq caption+hashtags, then archive (Drive + delivered/).

# Exit codes: 0 delivered | 2 processed but not shippable | 1 hard error
# """
# import subprocess
# import sys
# from pathlib import Path

# HERE = Path(__file__).resolve().parent
# PY   = sys.executable

# FIX_STAGE = {
#     "loudness -16..-12 LUFS":  "finalize",
#     "true peak <= -0.5 dBTP":  "finalize",
#     "no caption overlap":      "captions",
#     "music bed audible":       "sound",
#     "no black frames":         "effects",
# }
# BLACK_CASCADE = ["effects", "captions", "finalize"]
# MAX_ATTEMPTS  = 2


# def run(cmd, **kw):
#     print(f"\n  $ {' '.join(str(c) for c in cmd)}")
#     return subprocess.run(cmd, **kw).returncode


# def pipeline(client_path, stage=None, restart=False):
#     cmd = [PY, str(HERE / "pipeline.py"), str(client_path)]
#     if restart:
#         cmd.append("--restart")
#     if stage:
#         cmd += ["--stage", stage]
#     else:
#         cmd.append("--all")
#     return run(cmd)


# def qc(client_path):
#     run([PY, str(HERE / "qc_check.py"), str(client_path)])
#     review = client_path / "edit" / "REVIEW.md"
#     if not review.exists():
#         return "FATAL", []
#     text = review.read_text(encoding="utf-8")
#     first = text.splitlines()[0] if text.splitlines() else ""
#     verdict = first.split("----")[-1].strip() if "----" in first else "UNKNOWN"
#     failed = []
#     in_failed = False
#     for ln in text.splitlines():
#         if ln.startswith("## FAILED"):
#             in_failed = True; continue
#         if in_failed and ln.startswith("- "):
#             failed.append(ln[2:].split(":")[0].strip())
#         elif in_failed and ln.startswith("##"):
#             break
#     return verdict, failed


# def is_shippable(verdict):
#     return verdict.startswith("SHIPPABLE")


# def self_correct(client_path, failed):
#     for check in failed:
#         if check == "caption tags well-formed":
#             print(f"  [STOP] '{check}' = reel_captions.py corrupted. Will NOT ship.")
#             return False
#         if check == "duration 12-90s":
#             print(f"  [skip] '{check}' is footage length - cannot fix, will flag.")
#             continue
#         if check == "no black frames":
#             print(f"  [fix] '{check}' -> cascade {BLACK_CASCADE}")
#             for st in BLACK_CASCADE:
#                 pipeline(client_path, stage=st)
#             continue
#         stage = FIX_STAGE.get(check)
#         if stage:
#             print(f"  [fix] '{check}' -> re-run stage '{stage}'")
#             pipeline(client_path, stage=stage)
#         else:
#             print(f"  [?] no known fix for '{check}' - leaving as is.")
#     return True


# def main():
#     if len(sys.argv) < 2:
#         sys.exit("Usage: python run_client.py <CLIENT>")
#     client = sys.argv[1]
#     client_path = HERE / "projects" / client

#     print(f"\n{'='*64}\n  RUN CLIENT: {client}\n{'='*64}")

#     # 1. FETCH
#     print("\n[1/5] FETCH from Google Drive")
#     if run([PY, str(HERE / "rclone_sync.py"), "fetch", client]) != 0:
#         print(f"\n[FAIL] fetch failed for {client}. Aborting (exit 1)."); sys.exit(1)
#     raw = client_path / "raw"
#     clips = list(raw.glob("*.mp4")) + list(raw.glob("*.mov")) if raw.exists() else []
#     if not clips:
#         print(f"\n[skip] No clips for {client} (empty inbox)."); sys.exit(0)
#     print(f"  fetched {len(clips)} clip(s).")

#     # 2. PIPELINE
#     print("\n[2/5] PIPELINE (all stages, clean restart)")
#     edit = client_path / "edit"
#     if edit.exists():
#         import shutil; shutil.rmtree(edit)
#     if pipeline(client_path, restart=True) != 0:
#         print(f"\n[FAIL] pipeline crashed for {client} (exit 1)."); sys.exit(1)

#     # 3. QC + SELF-CORRECT
#     print("\n[3/5] QC + self-correct")
#     verdict, failed = qc(client_path)
#     print(f"  QC verdict: {verdict} | failed checks: {failed}")
#     attempt = 0
#     while failed and not is_shippable(verdict) and attempt < MAX_ATTEMPTS:
#         attempt += 1
#         print(f"\n  -- self-correct attempt {attempt}/{MAX_ATTEMPTS} --")
#         if not self_correct(client_path, failed):
#             break
#         verdict, failed = qc(client_path)
#         print(f"  QC verdict: {verdict} | failed checks: {failed}")

#     # 4. CAPTION (only if shippable - generate before archive so link merges in)
#     if is_shippable(verdict):
#         print("\n[4/5] CAPTION (Groq)")
#         run([PY, str(HERE / "caption_gen.py"), client])

#     # 5. ARCHIVE (only if shippable)
#     print("\n[5/5] ARCHIVE decision")
#     if is_shippable(verdict):
#         print(f"  {verdict} -> archiving to Drive + delivered/")
#         if run([PY, str(HERE / "rclone_sync.py"), "archive", client]) != 0:
#             print(f"\n[WARN] archive failed for {client} after a shippable reel. Exit 1.")
#             sys.exit(1)
#         print(f"\n[DONE] {client} delivered. (exit 0)")
#         sys.exit(0)
#     else:
#         print(f"  {verdict} -> NOT shippable. Skipping archive.")
#         print(f"  Clips remain in the Drive inbox for {client} - next run will retry.")
#         print(f"\n[NOT DELIVERED] {client} did not pass QC. See projects/{client}/edit/REVIEW.md (exit 2).")
#         sys.exit(2)


# if __name__ == "__main__":
#     main()




"""
run_client.py - fully automated single-client reel run.

    fetch -> pipeline (6 stages) -> qc -> [self-correct] -> caption -> archive (only if shippable)

  - STRICT QC gate: if objective checks fail after retries, DO NOT archive.
  - Self-correction: re-run the relevant stage, max 2 attempts per issue.
  - On shippable: generate Groq caption+hashtags, then archive (Drive + delivered/).

Exit codes: 0 delivered | 2 processed but not shippable | 1 hard error
"""
import subprocess
import sys
from pathlib import Path

HERE = Path(__file__).resolve().parent
PY   = sys.executable

FIX_STAGE = {
    "loudness -16..-12 LUFS":  "finalize",
    "true peak <= -0.5 dBTP":  "finalize",
    "no caption overlap":      "captions",
    "music bed audible":       "sound",
    "no black frames":         "effects",
}
BLACK_CASCADE = ["effects", "captions", "finalize"]
MAX_ATTEMPTS  = 2


def run(cmd, **kw):
    print(f"\n  $ {' '.join(str(c) for c in cmd)}")
    return subprocess.run(cmd, **kw).returncode


def pipeline(client_path, stage=None, restart=False):
    cmd = [PY, str(HERE / "pipeline.py"), str(client_path)]
    if restart:
        cmd.append("--restart")
    if stage:
        cmd += ["--stage", stage]
    else:
        cmd.append("--all")
    return run(cmd)


def qc(client_path):
    run([PY, str(HERE / "qc_check.py"), str(client_path)])
    review = client_path / "edit" / "REVIEW.md"
    if not review.exists():
        return "FATAL", []
    text = review.read_text(encoding="utf-8")
    first = text.splitlines()[0] if text.splitlines() else ""
    verdict = first.split("----")[-1].strip() if "----" in first else "UNKNOWN"
    failed = []
    in_failed = False
    for ln in text.splitlines():
        if ln.startswith("## FAILED"):
            in_failed = True; continue
        if in_failed and ln.startswith("- "):
            failed.append(ln[2:].split(":")[0].strip())
        elif in_failed and ln.startswith("##"):
            break
    return verdict, failed


def is_shippable(verdict):
    return verdict.startswith("SHIPPABLE")


def self_correct(client_path, failed):
    for check in failed:
        if check == "caption tags well-formed":
            print(f"  [STOP] '{check}' = reel_captions.py corrupted. Will NOT ship.")
            return False
        if check == "duration 12-90s":
            print(f"  [skip] '{check}' is footage length - cannot fix, will flag.")
            continue
        if check == "no black frames":
            print(f"  [fix] '{check}' -> cascade {BLACK_CASCADE}")
            for st in BLACK_CASCADE:
                pipeline(client_path, stage=st)
            continue
        stage = FIX_STAGE.get(check)
        if stage:
            print(f"  [fix] '{check}' -> re-run stage '{stage}'")
            pipeline(client_path, stage=stage)
        else:
            print(f"  [?] no known fix for '{check}' - leaving as is.")
    return True


def main():
    if len(sys.argv) < 2:
        sys.exit("Usage: python run_client.py <CLIENT>")
    client = sys.argv[1]
    client_path = HERE / "projects" / client

    print(f"\n{'='*64}\n  RUN CLIENT: {client}\n{'='*64}")

    # 1. FETCH
    print("\n[1/5] FETCH from Google Drive")
    if run([PY, str(HERE / "rclone_sync.py"), "fetch", client]) != 0:
        print(f"\n[FAIL] fetch failed for {client}. Aborting (exit 1)."); sys.exit(1)
    raw = client_path / "raw"
    clips = list(raw.glob("*.mp4")) + list(raw.glob("*.mov")) if raw.exists() else []
    if not clips:
        print(f"\n[skip] No clips for {client} (empty inbox)."); sys.exit(0)
    print(f"  fetched {len(clips)} clip(s).")

    # 2. PIPELINE
    print("\n[2/5] PIPELINE (all stages, clean restart)")
    edit = client_path / "edit"
    if edit.exists():
        import shutil; shutil.rmtree(edit)
    if pipeline(client_path, restart=True) != 0:
        print(f"\n[FAIL] pipeline crashed for {client} (exit 1)."); sys.exit(1)

    # 3. QC + SELF-CORRECT
    print("\n[3/5] QC + self-correct")
    verdict, failed = qc(client_path)
    print(f"  QC verdict: {verdict} | failed checks: {failed}")
    attempt = 0
    while failed and not is_shippable(verdict) and attempt < MAX_ATTEMPTS:
        attempt += 1
        print(f"\n  -- self-correct attempt {attempt}/{MAX_ATTEMPTS} --")
        if not self_correct(client_path, failed):
            break
        verdict, failed = qc(client_path)
        print(f"  QC verdict: {verdict} | failed checks: {failed}")

    # 4. CAPTION (only if shippable - generate before archive so link merges in)
    if is_shippable(verdict):
        print("\n[4/5] CAPTION (Groq)")
        run([PY, str(HERE / "caption_gen.py"), client])

    # 5. ARCHIVE (only if shippable)
    print("\n[5/5] ARCHIVE decision")
    if is_shippable(verdict):
        print(f"  {verdict} -> uploading preview reel; clips stay in inbox until approval")
        if run([PY, str(HERE / "rclone_sync.py"), "preview", client]) != 0:
            print(f"\n[WARN] preview failed for {client} after a shippable reel. Exit 1.")
            sys.exit(1)
        print(f"\n[DONE] {client} delivered. (exit 0)")
        sys.exit(0)
    else:
        print(f"  {verdict} -> NOT shippable. Skipping archive.")
        print(f"  Clips remain in the Drive inbox for {client} - next run will retry.")
        print(f"\n[NOT DELIVERED] {client} did not pass QC. See projects/{client}/edit/REVIEW.md (exit 2).")
        sys.exit(2)


if __name__ == "__main__":
    main()