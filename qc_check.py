...
#         gaps = [vol(str(final), x, 0.5, "mean_volume:") for x in (d*0.3, d*0.6, d*0.9)]
#         ok("music bed audible", any(g>-42 for g in gaps), "gaps "+str([round(g) for g in gaps]))
#         WHOOSH_MIN_SPIKE = 3.0  # dB the seam must rise above pre-seam level to be audible
#         seam_spikes = []
#         weak = []
#         for c in cuts:
#             before = vol(str(final), max(0,c-0.6), 0.25, "max_volume:")
#             at = vol(str(final), max(0,c-0.12), 0.25, "max_volume:")
#             spike = at - before
#             seam_spikes.append(round(spike,1))
#             if spike < WHOOSH_MIN_SPIKE:
#                 weak.append(round(c,1))
#         if cuts:
#             ok("whoosh audible at seams", not weak,
#                "spikes "+str(seam_spikes)+" dB"+("" if not weak else "; weak at "+str(weak)+"s"))
#     failed=[c for c in checks if not c[1]]
#     verdict = "SHIPPABLE" if not failed and not flags else ("BLOCKED - objective fail" if failed else "SHIPPABLE (with human-eye flags)")
#     L=["# REVIEW  ----  "+verdict,"","final.mp4  "+str(round(d,1))+"s","","## Objective checks"]
#     for n,p,dd in checks: L.append("- ["+("x" if p else " ")+"] "+n+((" ("+dd+")") if dd else ""))
...
def main():
    ...
    if (edit/"bed.mp3").exists():
        gaps = [vol(str(final), x, 0.5, "mean_volume:") for x in (d*0.3, d*0.6, d*0.9)]
        ok("music bed audible", any(g>-42 for g in gaps), "gaps "+str([round(g) for g in gaps]))
        # Whoosh seam levels are REPORTED as a flag, not a hard fail: peak-vs-peak
        # can't reliably distinguish "audible" from "inaudible" against a loud bed,
        # and re-running sound is deterministic so retrying can't fix a level issue.
        seam_spikes = []
        weak = []
        for c in cuts:
            before = vol(str(final), max(0,c-0.6), 0.25, "max_volume:")
            at = vol(str(final), max(0,c-0.12), 0.25, "max_volume:")
            spike = round(at - before, 1)
            seam_spikes.append(spike)
            if spike < 2.0:
                weak.append(round(c,1))
        if cuts and weak:
            flags.append("Whoosh weak/absent at seam(s) "+str(weak)+"s (spikes "
                         +str(seam_spikes)+" dB). Listen and re-run sound if needed.")
    failed=[c for c in checks if not c[1]]
    verdict = "SHIPPABLE" if not failed and not flags else ("BLOCKED - objective fail" if failed else "SHIPPABLE (with human-eye flags)")
    L=["# REVIEW  ----  "+verdict,"","final.mp4  "+str(round(d,1))+"s","","## Objective checks"]
    for n,p,dd in checks: L.append("- ["+("x" if p else " ")+"] "+n+((" ("+dd+")") if dd else ""))
